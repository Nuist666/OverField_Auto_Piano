...
```
.
├─ main_single.py            # 单人 / 原版 GUI 入口
├─ main_multi.py             # 多人模式入口（去和弦 + 多音分散）
├─ src/
│  ├─ app.py                 # 基础 GUI 框架
│  ├─ app_single.py          # 单人模式界面与解析
│  ├─ app_multi.py           # 多人模式界面与偏移逻辑
│  ├─ event.py               # 事件数据结构
│  ├─ key_sender.py          # 按键发送封装（pyautogui）
│  └─ player.py              # 播放线程（调度按下/释放）
├─ utils/
│  ├─ constant.py            # 键位与正则常量
│  ├─ parse.py               # 乐谱解析 & 多人预处理
│  └─ midi2lrcp.py           # MIDI 转 .lrcp 示例脚本（示例内置路径）
├─ example/
│  ├─ lrcp/                  # 示例 .lrcp 乐谱
│  │   ├─ 一闪一闪亮晶晶.lrcp
│  │   └─ 卡农.lrcp
...
└─ README.md
```
> 说明：原先文档中出现的 `play_piano.py` / `play_piano_multi.py` / 根目录 `midi2lrcp.py` / `main.py` 已在最新重构中被上述文件取代。请以本节为准。
