...
## 📂 文件结构
- `main_single.py` → 单人模式（完整旋律含和弦）  
- `main_multi.py` → 多人模式（去和弦 + 多音时间分散）  
- `example/lrcp/一闪一闪亮晶晶.lrcp` → 示例乐谱文件（《一闪一闪亮晶晶》）  
- `example/lrcp/卡农.lrcp` → 示例乐谱文件（《卡农》）  
- `example/mid/卡农.mid` → 示例mid文件（《卡农》）
- `utils/midi2lrcp.py` → mid文件转换工具
- `README.md` → 使用说明书  
...
5.  mid格式转换：
   ...
   ```
   - 正常情况下会提示`已生成: xx.lrcp`，生成 `.lrcp` 乐谱
...
### 策略
为提升他人端听感，`main_multi.py` 做以下退化预处理（不修改源谱文件）：
...
## 📌 示例演奏
- 运行 `main_single.py` 选择 `example`文件夹下的 `mid` 文件或者 `lrcp` 文件  
- 或在多人模式下运行 `main_multi.py` 体验分散后的主旋律  
