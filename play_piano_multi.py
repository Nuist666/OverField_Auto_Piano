...
class KeySender:
    ...
    def press(self, k: str):
        ...
        if cnt==1:
            try:
                if USE_KEYBOARD_LIB: keyboard.press(k)
                else: pyautogui.keyDown(k)
    ...
    def release(self, k: str):
        ...
        if cnt==0:
            try:
                if USE_KEYBOARD_LIB: keyboard.release(k)
...
class Player(threading.Thread):
    def __init__(self, events: List[SimpleEvent], start_delay: float, global_latency_ms: int, speed_ratio: float, on_done):
    ...
    def run(self):
        try:
            ...
            scaled: List[SimpleEvent] = [SimpleEvent(e.start/self.speed_ratio, e.end/self.speed_ratio, e.key) for e in self.events]
            ...
            while idx < len(actions) and not self._stop.is_set():
                ...
                if wait>0:
                    ...
                    continue
                _, typ, k = actions[idx]
                if typ=='press': key_sender.press(k)
                else: key_sender.release(k)
                idx+=1
        finally:
...
class App:
    def __init__(self, root: tk.Tk):
        ...
        file_bar = tk.Frame(frm); file_bar.pack(fill='x')
        tk.Button(file_bar, text='载入乐谱', command=self.load_score).pack(side='left')
        self.lbl_file = tk.Label(file_bar, text='未载入'); self.lbl_file.pack(side='left', padx=8)

        params = tk.LabelFrame(frm, text='参数'); params.pack(fill='x', pady=8)
        tk.Label(params, text='速度比例:').grid(row=0,column=0,sticky='e')
        self.ent_speed = tk.Entry(params, width=8); self.ent_speed.insert(0,'1.0'); self.ent_speed.grid(row=0,column=1,sticky='w',padx=4)
        tk.Label(params, text='起始倒计时(s):').grid(row=0,column=2,sticky='e')
        self.ent_countin = tk.Entry(params, width=8); self.ent_countin.insert(0,'2.0'); self.ent_countin.grid(row=0,column=3,sticky='w',padx=4)
        tk.Label(params, text='全局延迟(ms):').grid(row=0,column=4,sticky='e')
        self.ent_latency = tk.Entry(params, width=8); self.ent_latency.insert(0,'0'); self.ent_latency.grid(row=0,column=5,sticky='w',padx=4)
        tk.Label(params, text='多音偏移(ms):').grid(row=1,column=0,sticky='e')
        self.ent_offsets = tk.Entry(params, width=20); self.ent_offsets.insert(0,'-15,0,15'); self.ent_offsets.grid(row=1,column=1,columnspan=3,sticky='w',padx=4)
